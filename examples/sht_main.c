...
#define RECORDS_NUM 193 // you can change it if you want
...
int main() {
  ...
  int surnameFrequency[12];

  for (i=0 ; i < 12 ; i++) {
    surnameFrequency[i]= 0;
  }
  ...
  printf("RUN PrintAllEntries FOR 1st SECONDARY FILE, WITH KEY \"Gaitanis\"\n");
  CALL_OR_DIE(SHT_PrintAllEntries(sindexDesc1, "Gaitanis"));
  printf("********* PrintAllEntries FOR 1st SECONDARY (\"Gaitanis\") COMPLETED*******\n\n");
  ...
  CALL_OR_DIE(SHT_PrintAllEntries(sindexDesc1, "Sarlas"));
  printf("\"Sarlas\" was found in the secondary file\n\n");

  

  printf("RUN INNER JOIN FOR THE TWO SECONDARY FILES FOR \"Gaitanis\"\n");
  CALL_OR_DIE(SHT_InnerJoin( sindexDesc1, sindexDesc2, "Gaitanis"));
  printf("********* INNER JOIN FOR \'Gaitanis\" COMPLETED*******\n\n");
